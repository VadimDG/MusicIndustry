...
namespace boilerplate.api.core.Extensions
{
    public static class DependencyExtension
    {
        public static void AddApiClients(this IServiceCollection services, ApiConfig apiConfig)
        {
            ...
            services.AddHttpClient(nameof(IPlatformContactClient),
                    c =>
                    {
                        c.BaseAddress = new Uri(apiConfig.BaseUrl);
                    })
                .AddTypedClient(c => Refit.RestService.For<IPlatformContactClient>(c));
            services.AddHttpClient(nameof(IMusicianContactClient),
                    c =>
                    {
                        c.BaseAddress = new Uri(apiConfig.BaseUrl);
                    })
                .AddTypedClient(c => Refit.RestService.For<IMusicianContactClient>(c));
            services.AddHttpClient(nameof(IMusicLabelContactClient),
                    c =>
                    {
                        c.BaseAddress = new Uri(apiConfig.BaseUrl);
                    })
                .AddTypedClient(c => Refit.RestService.For<IMusicLabelContactClient>(c));
        }
    }
}
